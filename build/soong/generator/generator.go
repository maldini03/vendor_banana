...
func init() {
	android.RegisterModuleType("banana_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = bananaExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = bananaExpandVariables(ctx, dep_file)
	...
	cmd := bananaExpandVariables(ctx, String(g.properties.Cmd))
